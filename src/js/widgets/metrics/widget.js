  ...
  function (
    ...
    var ContainerView = Marionette.LayoutView.extend({
      ...
      generatePDF : function(){
        ...
        $copy.find("i.icon-help").remove();
        $copy.find(".close-widget").remove();
        $copy.find(".graph-tab").remove();
        d3.select($copy[0]).selectAll("#indices g.nv-group").style({"fill-opacity" : "0"});

        $copy.appendTo("body");

        $copy.find(".metrics-svg").each(function(index, svgTag){

          d3.select(svgTag).selectAll("text").style("font-size", "16px");
    ...
    var MetricsWidget = BaseWidget.extend({
      ...
      processResponse: function (response) {
        ...
        if (response instanceof ApiResponse){
          ...
          this.view.model.set({
              numFound : parseInt(response.get("response.numFound")),
              rows :  parseInt(response.get("responseHeader.params.rows")),
              q : this._currentQuery.get("q")[0]
            });
